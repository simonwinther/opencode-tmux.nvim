...
-- so selected code can be dropped straight into a prompt with context
---@return string
...
function M.resolve(text, this_ctx)
  local replacements = {
    ["@this"] = function() return this_ctx or M.current_line() end,
    ["@buffer"] = function() return M.buffer() end,
    ["@diagnostics"] = function() return M.diagnostics() end,
    ["@diff"] = function() return M.diff() end,
  }
